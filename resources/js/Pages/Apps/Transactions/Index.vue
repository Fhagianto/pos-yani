...
if (product.value.stock < qty.value) {
        Swal.fire({
            title: 'Error!',
            text: 'Kuantitas melebihi stok yang tersedia untuk produk ' + product.value.title + '.',
            icon: 'error',
    ...
    // Ambil semua cart untuk pengecekan stok
    const carts = props.carts; // Ambil cart dari props

    // Cek setiap item di cart
    for (const cart of carts) {
        if (cart.qty > cart.product.stock) {
            Swal.fire({
                title: 'Error!',
                text: 'Kuantitas produk ' + cart.product.title + ' melebihi stok yang tersedia.',
                icon: 'error',
                confirmButtonText: 'OK'
            });
            return; // Hentikan proses jika ada kuantitas yang melebihi stok
        }
    }

    // Jika semua pengecekan lulus, lanjutkan untuk mengirim transaksi
