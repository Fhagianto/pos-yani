...
export default {
    ...
    setup(props) { // Add props as a parameter
        const search = ref('' || (new URL(document.location)).searchParams.get('q'));

        const handleSearch = () => {
            router.get('/apps/products', { q: search.value });
        };

        const downloadPDF = () => {
            const pdf = new jsPDF();
            const element = document.getElementById('table-responsive'); // Ensure this ID is correct
            ...
            html2canvas(element).then((canvas) => {
                ...
                // Add titles and barcodes for each product
